...
namespace ct {
    ...
    namespace binary_impl {
        ...
        public:
            ...
            explicit binary_expression(LeftOperand&& left_operand_,
                RightOperand&& right_operand_,
                std::shared_ptr<binary_operator> binary_op_)
              : left_operand(std::move(left_operand_))
              , right_operand(std::move(right_operand_))
              , binary_op(binary_op_)
            {
            }
            ...
            auto operator()() const
            {
                if constexpr (ct::traits::is_vec_type<LeftOperand,
                                  RightOperand>::value)
                {
                    return create_vec_ast();
                }
                else if constexpr (ct::traits::is_mat_type<LeftOperand,
                                       RightOperand>::value)
                {
                    return create_mat_ast();
                }
            ...
            std::size_t size() const
            {
                return left_operand.size();
            }
            std::size_t rows() const
            {
                return left_operand.rows();
            }
            std::size_t cols() const
            {
                return left_operand.cols();
            }
        ...
        private:
            ...
            std::vector<ct::vec_impl::vec_node> create_vec_ast() const
            {
                ...
                std::copy(
                    left_ast.begin(), left_ast.end(), std::back_inserter(ast));
                std::copy(right_ast.begin(), right_ast.end(),
                    std::back_inserter(ast));
